...
/**
 * Load the automaton config from disk or environment variables.
...
export function loadConfig(): AutomatonConfig | null {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      const apiKey = raw.conwayApiKey || process.env.CONWAY_API_KEY || loadApiKeyFromConfig()
  ...
  // If no config file, try to load from environment variables
...
export function loadConfigFromEnv(): AutomatonConfig | null {
  const name = process.env.AUTOMATON_NAME
  const genesisPrompt = process.env.AUTOMATON_GENESIS_PROMPT
  const creatorAddress = process.env.AUTOMATON_CREATOR_ADDRESS
  ...
  return {
    ...
    conwayApiKey: apiKey || loadApiKeyFromConfig(),
    conwayApiUrl: process.env.CONWAY_API_URL || DEFAULT_CONFIG.conwayApiUrl,
    openaiApiKey: process.env.OPENAI_API_KEY,
    anthropicApiKey: process.env.ANTHROPIC_API_KEY,
    ollamaBaseUrl: process.env.OLLAMA_BASE_URL,
    inferenceModel: process.env.AUTOMATON_INFERENCE_MODEL || DEFAULT_CONFIG.inferenceModel,
    maxTokensPerTurn: process.env.AUTOMATON_MAX_TOKENS_PER_TURN
      ? parseInt(process.env.AUTOMATON_MAX_TOKENS_PER_TURN)
